...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, keywords_ids=None):
        if keywords_ids is None:
            ...
            seq_length = keywords_ids.size(1)
            if position_ids is None:
                position_ids = torch.arange(seq_length, dtype=torch.long, device=keywords_ids.device)
                position_ids = position_ids.unsqueeze(0).expand_as(keywords_ids)
            words_embeddings = self.word_embeddings(keywords_ids)
            position_embeddings = self.position_embeddings(position_ids)
...
class TourHistoryEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        feature_dim = 768
        angle_feature_dim = 4
        self.txt_hist_linear = nn.Linear(feature_dim, feature_dim)
        self.txt_hist_layer_norm = BertLayerNorm(feature_dim, eps=1e-12)
        self.txt_ang_linear = nn.Linear(angle_feature_dim, feature_dim)
        self.txt_ang_layer_norm = BertLayerNorm(feature_dim, eps=1e-12)

        self.neighbour_depth_embedding = nn.Embedding(10, feature_dim)
        self.layer_norm = BertLayerNorm(feature_dim, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        
    def forward(self, txt_hist_feat, txt_ang_feat, neighbour_depth=None):
        transformed_txt_hist = self.txt_hist_layer_norm(self.txt_hist_linear(txt_hist_feat))
        transformed_txt_ang = self.txt_ang_layer_norm(self.txt_ang_linear(txt_ang_feat))
        embeddings = transformed_txt_hist + transformed_txt_ang
        if neighbour_depth is not None:
            embeddings = embeddings + self.neighbour_depth_embedding(neighbour_depth)
        embeddings = self.layer_norm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
...
class NavCMT_IVLN_OV(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.img_embeddings = ImageEmbeddings(config)

        self.keywords_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        self.tour_hist_embeddings = TourHistoryEmbeddings(config)
    ...
    def _txt_hist_variable(self, txt_memory):

        keywords = [_[0] for _ in txt_memory]
        keywords_num = [len(keywords[i]) for i in range(len(keywords))]
        if sum(keywords_num) == 0:
            return None, None, None, None
        
        keywords_depths = [_[2] for _ in txt_memory] # (batch size, keywords num)
        keywords_depths = [torch.tensor(_).long().cuda() for _ in keywords_depths]
        keywords_depths = [_ + 1 for _ in keywords_depths]
        
        angle_embeds = [torch.from_numpy(np.stack(_[3], 0)).cuda() if len(_[3]) > 0 else torch.empty(0, 4).cuda() for _ in txt_memory]
        

        keywords_token = [self.keywords_tokenizer(_)['input_ids'] if len(_) > 0 else [] for _ in keywords]
        for i in range(len(keywords_token)):
            assert len(keywords_token[i]) == len(keywords[i]), 'Keywords: {}   Tokens: {}'.format(keywords[i], keywords_token[i])

        keywords_token_flatten = [i for j in keywords_token for i in j]
        seq_lengths = [len(keywords_token_flatten[i]) for i in range(len(keywords_token_flatten))] # keyword length
        seq_tensor = np.zeros((len(keywords_token_flatten), max(seq_lengths)), dtype=np.int64)
        mask = np.zeros((len(keywords_token_flatten), max(seq_lengths)), dtype=np.bool)
        for i, keywords_token in enumerate(keywords_token_flatten):
            seq_tensor[i, :seq_lengths[i]] = keywords_token
            mask[i, :seq_lengths[i]] = True
        seq_tensor = torch.from_numpy(seq_tensor).cuda()
        mask = torch.from_numpy(mask).cuda()

        keywords_embeds_flatten = self.embeddings(keywords_ids=seq_tensor, input_ids=None)

        keywords_embeds_flatten = keywords_embeds_flatten[:, 0, :]
        keywords_embeds = list()
        begin = 0
        for i in range(len(keywords_num)):
            keywords_embeds.append(keywords_embeds_flatten[begin:begin+keywords_num[i]])
            begin += keywords_num[i]

        max_keywords_num = max(keywords_num)
        for i in range(len(keywords_embeds)):
            zero_padded = torch.zeros((max_keywords_num-keywords_num[i], keywords_embeds[i].size(-1))).cuda()
            keywords_embeds[i] = torch.cat([keywords_embeds[i], zero_padded], 0)
            zero_padded = torch.zeros((max_keywords_num-keywords_num[i], angle_embeds[i].size(-1))).cuda()
            angle_embeds[i] = torch.cat([angle_embeds[i], zero_padded], 0)

            zero_padded = torch.zeros(max_keywords_num-keywords_num[i]).long().cuda()
            keywords_depths[i] = torch.cat([keywords_depths[i], zero_padded], 0)
            assert keywords_embeds[i].shape[0] == angle_embeds[i].shape[0]
            assert keywords_embeds[i].shape[0] == keywords_depths[i].shape[0]
        
        keywords_embeds = torch.stack(keywords_embeds, 0)
        angle_embeds = torch.stack(angle_embeds, 0)
        keywords_depths = torch.stack(keywords_depths, 0)
        return keywords_embeds, angle_embeds, keywords_num, keywords_depths
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
                ...
                ob_embeds=None, ob_masks=None, txt_memory=None):
        # text embedding            
        if mode == 'language':
        ...
        # cross-modal encoding per step
            ...
            if txt_memory is not None:
                txt_hist_embeds, txt_hist_ang_embeds, txt_hist_num, txt_hist_depths = self._txt_hist_variable(txt_memory=txt_memory)
            else:
                txt_hist_embeds, txt_hist_ang_embeds, txt_hist_num, txt_hist_depths = None, None, None, None
            if self.config.ablation_depth and txt_hist_depths is not None:
                txt_hist_depths = torch.zeros_like(txt_hist_depths)
                if self.config.shuffle:
                    perm = torch.randperm(txt_hist_embeds.shape[1])
                    txt_hist_embeds = txt_hist_embeds[:,perm,:]
                    txt_hist_ang_embeds = txt_hist_ang_embeds[:,perm,:]
            if self.config.ablation_angle and txt_hist_ang_embeds is not None:
                txt_hist_ang_embeds = torch.zeros_like(txt_hist_ang_embeds)
                if self.config.shuffle:
                    perm = torch.randperm(txt_hist_embeds.shape[1])
                    txt_hist_embeds = txt_hist_embeds[:,perm,:]
                    txt_hist_depths = txt_hist_depths[:,perm]
            
            if txt_hist_embeds is not None:
                txt_hist_embeds = txt_hist_embeds.detach()
                txt_hist_ang_embeds = txt_hist_ang_embeds.detach()
                graph_hist_embeds = self.tour_hist_embeddings(txt_hist_embeds, txt_hist_ang_embeds, txt_hist_depths)
                graph_hist_mask = length2mask(txt_hist_num).logical_not()

                extended_graph_hist_masks = graph_hist_mask.unsqueeze(1).unsqueeze(2)
                extended_graph_hist_masks = extended_graph_hist_masks.to(dtype=self.dtype)
                extended_graph_hist_masks = (1.0 - extended_graph_hist_masks) * -10000.0
            ...
            if txt_hist_embeds is not None:
                txt_max_len = txt_embeds.size(1)
                txt_graph_embeds = torch.cat([txt_embeds, graph_hist_embeds], 1)
                extended_txt_graph_masks = torch.cat([extended_txt_masks, extended_graph_hist_masks], -1)
            
            if self.config.no_lang_ca: # false
                assert False
                all_txt_embeds = txt_embeds
            for l, layer_module in enumerate(self.encoder.x_layers):
                ...
                if txt_hist_embeds is None:
                    txt_embeds, hist_ob_embeds = layer_module(
                        txt_embeds, extended_txt_masks, 
                        hist_ob_embeds, extended_hist_ob_masks,
                    )
                else:
                    txt_graph_embeds, hist_ob_embeds = layer_module(
                        txt_graph_embeds, extended_txt_graph_masks, 
                        hist_ob_embeds, extended_hist_ob_masks,
                    )
            ...
            if txt_hist_embeds is not None:
                txt_embeds = txt_graph_embeds[:, :txt_max_len]
                graph_hist_embeds = txt_graph_embeds[:, txt_max_len:]
